...
type Payload struct {
	...
	// null persona/binding/set would; if the face is then worn nowhere, it is retired as
	// persona/profile/retire would a pool face, never deleted, so its history survives.
	// Absent means the binding lasts until changed. For the face worn for one weekend — a
	// conference, a listing — so that ending it is a default rather than a discipline. A
	// maintainer MUST refuse an `until` that is not in the future, and one given with a null
	// `profileId`.
