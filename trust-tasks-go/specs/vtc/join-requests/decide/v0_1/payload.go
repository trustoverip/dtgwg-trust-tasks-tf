...
package vtcjoinrequestsdecidev0_1
...
// ResponseStatus The request's post-decision state; echoes the decision.
...
// Response The success response to a vtc/join-requests/decide request. Carried in a Trust
...
type Response struct {
	...
	// The membership credential issued by this decision, delivered inline. Present only where
	// the decision admitted the applicant. Inline because the alternative is worse: the
	// applicant would poll for a status, learn they were admitted, and then fetch the
	// credential separately — a second round trip whose only purpose is to collect something
	// the community already had in hand when it decided.
	Vmc *map[string]json.RawMessage `json:"vmc,omitempty"`

	// The endorsement credential carrying the role this decision granted, delivered inline
	// alongside `vmc` and on the same reasoning. Present only where the decision admitted the
	// applicant and granted a role.
...
// PayloadDecision The operator's decision. `approved` admits the applicant as a member;
// `rejected` refuses them.
...
const (
	...
	PayloadDecisionRejected PayloadDecision = "rejected"
)

// Payload VTC Join-Requests Decide — payload
type Payload struct {
	...
	// Optional operator rationale, recorded in the audit trail alongside the decision. Most
	// useful with `rejected`.
	Reason *string `json:"reason,omitempty"`
	Ext    *Ext    `json:"ext,omitempty"`
}

// TypeURI is the Trust Task type URI this package's Payload is carried under.
const TypeURI = "https://trusttasks.org/spec/vtc/join-requests/decide/0.1"

// ResponseTypeURI is the success-response form of TypeURI (SPEC §4.4.1).
const ResponseTypeURI = "https://trusttasks.org/spec/vtc/join-requests/decide/0.1#response"
...
const ResponsePayloadSchemaJSON = "{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\",\"$ref\":\"#/$defs/Response\",\"$defs\":{\"Response\":{\"$anchor\":\"response\",\"title\":\"VTC Join-Requests Decide — response payload\",\"description\":\"The success response to a vtc/join-requests/decide request. Carried in a Trust Task document whose type is https://trusttasks.org/spec/vtc/join-requests/decide/0.1#response.\",\"type\":\"object\",\"additionalProperties\":false,\"required\":[\"requestId\",\"status\"],\"properties\":{\"requestId\":{\"type\":\"string\",\"minLength\":1},\"status\":{\"type\":\"string\",\"enum\":[\"approved\",\"rejected\"],\"description\":\"The request's post-decision state; echoes the decision.\"},\"vmc\":{\"type\":[\"object\",\"null\"],\"description\":\"The membership credential issued by this decision, delivered inline. Present only where the decision admitted the applicant.\\n\\nInline because the alternative is worse: the applicant would poll for a status, learn they were admitted, and then fetch the credential separately — a second round trip whose only purpose is to collect something the community already had in hand when it decided.\"},\"roleVec\":{\"type\":[\"object\",\"null\"],\"description\":\"The endorsement credential carrying the role this decision granted, delivered inline alongside `vmc` and on the same reasoning. Present only where the decision admitted the applicant and granted a role.\"},\"ext\":{\"$ref\":\"#/$defs/Ext\"}}},\"Ext\":{\"title\":\"Ext\",\"description\":\"Vendor-namespaced extension object per SPEC.md §4.5.1. Each immediate key MUST be a reverse-DNS namespace; structure under each namespace is opaque to the framework.\",\"type\":\"object\",\"minProperties\":1,\"additionalProperties\":true,\"propertyNames\":{\"pattern\":\"^[a-z][a-z0-9-]*(\\\\.[a-z0-9-]+)+$\"}}}}"
