...
//!
//! Spec slug: `rooms/owner/issue-authority`. Version: `0.1`.
...
pub struct Payload {
    ...
    pub actions: ::std::vec::Vec<PayloadActionsItem>,
...
impl ::std::fmt::Display for PayloadActionsItem {
...
impl ::std::str::FromStr for PayloadActionsItem {
    type Err = self::error::ConversionError;
    fn from_str(value: &str) -> ::std::result::Result<Self, self::error::ConversionError> {
        match value {
            "read" => Ok(Self::Read),
...
impl ::std::convert::TryFrom<&str> for PayloadActionsItem {
    type Error = self::error::ConversionError;
    fn try_from(value: &str) -> ::std::result::Result<Self, self::error::ConversionError> {
        value.parse()
    }
}
impl ::std::convert::TryFrom<&::std::string::String> for PayloadActionsItem {
    type Error = self::error::ConversionError;
    fn try_from(
        value: &::std::string::String,
    ) -> ::std::result::Result<Self, self::error::ConversionError> {
...
pub mod builder {
    ...
    pub struct Payload {
        actions: ::std::result::Result<
            ::std::vec::Vec<super::PayloadActionsItem>,
            ::std::string::String,
        >,
    ...
    impl Payload {
        ...
        where
            T: ::std::convert::TryInto<::std::vec::Vec<super::PayloadActionsItem>>,
...
impl crate::Payload for Payload {
    const TYPE_URI: &'static str = "https://trusttasks.org/spec/rooms/owner/issue-authority/0.1";
...
mod conformance {
    ...
    fn rejects_invalid_examples() {
        ...
        let fixtures: &[(&str, &str)] = &[
            ...
            (
                "An empty actions list. It confers nothing rather than everything — but those are exactly the two readings a careless consumer might choose between, so the input is refused rather than the choice left open.",
                "{\n  \"actions\": [],\n  \"roomId\": \"did:webvh:example.com:rooms:northwind\",\n  \"signingKeyId\": \"room-northwind-signing\",\n  \"subject\": \"did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK\"\n}",
            ),
            (
                "No actions at all. Same reasoning as the empty list, and the more likely typo.",
                "{\n  \"roomId\": \"did:webvh:example.com:rooms:northwind\",\n  \"signingKeyId\": \"room-northwind-signing\",\n  \"subject\": \"did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK\"\n}",
            ),
            (
                "An action outside the enum. `delete` is not a room verb — retraction is a curation judgement (`curate`) and purging is a host operation, so a grant naming it would confer something no host implements.",
                "{\n  \"actions\": [\n    \"read\",\n    \"delete\"\n  ],\n  \"roomId\": \"did:webvh:example.com:rooms:northwind\",\n  \"signingKeyId\": \"room-northwind-signing\",\n  \"subject\": \"did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK\"\n}",
            ),
            (
                "actions as a bare string. One verb is still a list; accepting a string would make `\"read\"` and `[\"read\"]` two spellings of one grant, and a verifier that normalised only one of them would disagree with its peers.",
                "{\n  \"actions\": \"read\",\n  \"roomId\": \"did:webvh:example.com:rooms:northwind\",\n  \"signingKeyId\": \"room-northwind-signing\",\n  \"subject\": \"did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK\"\n}",
            ),
        ];
