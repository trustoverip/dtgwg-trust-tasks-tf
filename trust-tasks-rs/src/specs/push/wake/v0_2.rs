...
//!
//! Spec slug: `push/wake`. Version: `0.2`.
...
pub enum ResponseStatus {
    ...
    Delivered,
    #[serde(rename = "tokenUnregistered")]
    TokenUnregistered,
}
impl ::std::fmt::Display for ResponseStatus {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        match *self {
            ...
            Self::TokenUnregistered => f.write_str("tokenUnregistered"),
        }
    }
}
impl ::std::str::FromStr for ResponseStatus {
    ...
    fn from_str(value: &str) -> ::std::result::Result<Self, self::error::ConversionError> {
        match value {
            ...
            "tokenUnregistered" => Ok(Self::TokenUnregistered),
            _ => Err("invalid value".into()),
        }
    }
}
...
pub const ERROR_CODES: &[crate::DeclaredErrorCode] = &[
    ...
    error_codes::TOKEN_UNREGISTERED,
];
...
/// declared.
pub mod error_codes {
    /// `push/wake:unknownHandle`
    ///
    /// No such handle at this gateway.
    ///
    ...
    ///
    /// The authenticated trigger DID is not on the handle's VTA-provisioned allowlist.
    ///
    /// Declared `retryable: false`.
    ...
    ///
    /// The push service reports the handle's token permanently unregistered. The gateway drops the token; the device must re-register (push/register). The queued message remains for the consumer's next voluntary pickup.
    ///
    /// Declared `retryable: false`.
