...
pub mod error_codes {
    ...
    ///
    /// The entry's `secretKind` has no DID-based signing identity (`password`, `passkey`, `oauthTokens`, `bearerToken`, `sshKey`, `custom`). Only `didSelfIssued` and `didcommPeer` entries can sign Trust Tasks.
    ///
    /// Declared `retryable: false`.
