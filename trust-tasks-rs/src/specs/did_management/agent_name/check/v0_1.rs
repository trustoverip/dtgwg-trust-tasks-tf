...
pub const ERROR_CODES: &[crate::DeclaredErrorCode] =
    &[error_codes::INVALID_NAME, error_codes::UNKNOWN_DOMAIN];
...
/// declared.
pub mod error_codes {
    /// `did-management/agent-name/check:invalidName`
    ///
    /// The submitted `name` violates the agent-name grammar (length bounds, character set). Grammatical invalidity is an error, not unavailability — a reserved-list hit is reported in-band via `reserved: true`.
    ///
