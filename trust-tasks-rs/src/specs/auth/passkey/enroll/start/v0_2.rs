...
pub mod error_codes {
    ...
    ///
    /// The consumer requires re-authentication to add an authenticator but cannot mount a ceremony against the subject's existing credentials. Recovery is out of band.
    ///
    /// Declared `retryable: false`.
