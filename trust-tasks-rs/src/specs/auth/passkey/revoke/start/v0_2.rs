...
//!
//! Spec slug: `auth/passkey/revoke/start`. Version: `0.2`.
...
///Ask the auth service to begin revoking a passkey — the subject's own, or, for an administrator, another subject's. The response carries PublicKeyCredentialRequestOptions for a fresh user-verification ceremony over the producer's credentials; nothing is removed until the matching auth/passkey/revoke/finish presents the assertion.
///
/// <details><summary>JSON schema</summary>
///
/// ```json
///{
///  "$id": "https://trusttasks.org/spec/auth/passkey/revoke/start/0.2",
///  "title": "Payload",
///  "description": "Ask the auth service to begin revoking a passkey — the subject's own, or, for an administrator, another subject's. The response carries PublicKeyCredentialRequestOptions for a fresh user-verification ceremony over the producer's credentials; nothing is removed until the matching auth/passkey/revoke/finish presents the assertion.",
///  "type": "object",
///  "required": [
///    "credentialId"
///  ],
///  "properties": {
///    "credentialId": {
///      "description": "The credential to revoke, as returned by auth/passkey/list. Opaque to the producer.",
///      "type": "string",
///      "minLength": 1
///    },
///    "ext": {
...
///    },
///    "subject": {
///      "description": "New in 0.2. Whose credential it is, when the producer is not its owner: an administrator revoking another subject's credential. Absent: the producer's own. The user-verification ceremony is always over the PRODUCER's credentials — the person acting proves they are present, whoever owns the credential.",
///      "type": "string",
///      "minLength": 1
///    }
///  },
...
pub struct Payload {
    ...
    ///New in 0.2. Whose credential it is, when the producer is not its owner: an administrator revoking another subject's credential. Absent: the producer's own. The user-verification ceremony is always over the PRODUCER's credentials — the person acting proves they are present, whoever owns the credential.
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
...
///Server-issued re-authentication options. Carried in a Trust Task document whose type is https://trusttasks.org/spec/auth/passkey/revoke/start/0.2#response.
///
/// <details><summary>JSON schema</summary>
///
/// ```json
///{
///  "title": "Response",
///  "description": "Server-issued re-authentication options. Carried in a Trust Task document whose type is https://trusttasks.org/spec/auth/passkey/revoke/start/0.2#response.",
///  "type": "object",
///  "required": [
///    "revocationId",
///    "uvOptions"
...
impl crate::Payload for Payload {
    const TYPE_URI: &'static str = "https://trusttasks.org/spec/auth/passkey/revoke/start/0.2";
...
pub mod error_codes {
    ...
    ///
    /// No credential with this id is bound to the subject (the producer, or `payload.subject`). Consumers MUST return this for a credential belonging to a different subject as well, so the code cannot be used to probe whether an id exists elsewhere.
    ///
    ...
    ///
    /// The consumer cannot mount a user-verification ceremony for the producer — for example the producer holds no usable credential of a purpose that may authorise this revocation. Recovery is an administrator revoking for them, or out of band.
    ///
    /// Declared `retryable: false`.
    ...
    ///
    /// `payload.subject` names someone other than the producer, and the producer's authority does not extend to revoking that subject's credentials. Returned before any credential lookup, so it reveals nothing about the subject's credentials.
    ///
    /// Declared `retryable: false`.
...
mod conformance {
    ...
    fn rejects_invalid_examples() {
        ...
        let fixtures: &[(&str, &str)] = &[
            ...
            (
                "Empty subject — absent means the producer; empty means nothing.",
                "{\n  \"credentialId\": \"c3RlcHVwLWNyZWQtY2Fyb2w\",\n  \"subject\": \"\"\n}",
            ),
            (
                "Array of credential ids — one credential per ceremony, each removal separately verified.",
                "{\n  \"credentialId\": [\n    \"a\",\n    \"b\"\n  ]\n}",
            ),
            (
