...
pub mod error_codes {
    ...
    ///
    /// A `mergePatch` write named an address holding no live record. A patch has nothing to apply to; the caller must send a whole `value`.
    ///
    /// Declared `retryable: false`.
