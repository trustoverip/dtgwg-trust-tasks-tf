...
5. Return either a `#response`-variant *Trust Task document* (success) or a `trust-task-error` document (rejection) as the response body — both in JSON, both with `Content-Type: application/json`.
...
In every case where the body carries a Trust Task document — success or `trust-task-error` — the `Content-Type` **MUST** be `application/json`. The `202` and `204` answers carry no body and therefore no `Content-Type`.
