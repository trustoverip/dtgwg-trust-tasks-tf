...
| `suspended` | The response to the `trust-task-control` document that suspended it ([SPEC §11](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#11-task-control)), carried as an ordinary envelope of [§2](#2-document-carriage) and correlated per [§3.1](#31-thread-correlation). DIDComm expresses no transport-level counterpart: a closed WebSocket, an idle connection, or a pickup queue that stops draining is not suspension. |
