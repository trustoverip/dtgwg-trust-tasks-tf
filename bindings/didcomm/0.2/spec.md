...
When applying the §8.1 error-response routing rule under `identityMismatch`, the *consumer* **MUST** route its `trust-task-error` response to the transport-authenticated sender (the DID it actually authenticated via authcrypt) and **MUST NOT** route to the contested in-band `issuer`.
...
Error responses generated by the framework pipeline (a `trust-task-error` document) **SHOULD** be returned by packing the response back into a fresh authcrypt envelope addressed to the verified `sender_kid` of the originating message.

### 4.1 Lifecycle mapping

Required by [SPEC §9.1](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#91-what-a-transport-binding-specifies), which makes the mapping a **MUST** for every binding: each state of [SPEC §4.12 *Document Lifecycle*](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#412-document-lifecycle) is named below against the DIDComm event that carries it, or declared to have no counterpart here.

DIDComm is one-way and store-and-forward. There is no request/response pair at the transport layer, so **every** reply named below is itself a *Trust Task document* carried in a fresh envelope of [§2](#2-document-carriage)'s type, correlated per [§3.1](#31-thread-correlation) — not a DIDComm-layer signal. That distinction is the whole of this mapping: the transport has plenty of events of its own, and none of them is a document state.

| Lifecycle state | DIDComm counterpart |
|---|---|
| `received` | **No counterpart.** Unpacking the authcrypt envelope and deserialising `body` happen inside the *consumer* and emit nothing. In particular, a **mediator's** acknowledgement is not this state — see below. |
...
| `accepted` | **No counterpart**, unless the *Trust Task specification* defines a reply that acknowledges acceptance. The duplicate-execution record of [§6.1](#61-duplicate-execution-record) is written internally. Where the *consumer* is understood but **blocked**, the framework's reserved `trust-task-next-step` document is carried as an ordinary envelope of [§2](#2-document-carriage) and the document remains `accepted` with the exchange open. A DIDComm-layer `ack` or `problem-report` is **not** such a reply: this binding carries only its own envelope type, so a message of any other type is not a framework reply and **MUST NOT** be read as one. |
| `executing` | **No counterpart.** Nothing is emitted when work begins, and the *consumer* is under no obligation to emit anything until it has an outcome. |
| `suspended` | The response to the `trust-task-control` document that suspended it ([SPEC §12](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#12-task-control)), carried as an ordinary envelope of [§2](#2-document-carriage) and correlated per [§3.1](#31-thread-correlation). DIDComm expresses no transport-level counterpart: a closed WebSocket, an idle connection, or a pickup queue that stops draining is not suspension. |
| `responded` | A `<type>#response` document packed into a fresh authcrypt envelope of [§2](#2-document-carriage)'s type, addressed to the verified `sender_kid` of the originating message and correlated per [§3.1](#31-thread-correlation). Where the specification defines no success response, the framework's empty `{}` `#response` is the courtesy acknowledgement, sent the same way. |
| `errored` | A `trust-task-error` document, packed and addressed as [§4](#4-error-mapping) requires. The first, second and fifth rows of [§4](#4-error-mapping)'s table are the cases where **no** error can be returned — an anoncrypt or plaintext arrival, a foreign `type`, and a decryption or signature failure all stop before the pipeline, with no authenticated sender to route a reply to. There the failure is unobservable to the *producer*, exactly as the framework's `errored` row anticipates. |
| `cancelled` | The response to the `trust-task-control` document, for a *producer*-requested cancellation; a `trust-task-error` carrying `cancelled` where the *consumer* stopped of its own accord. **Nothing at the DIDComm layer expresses this** — see below. |
| `expired` | A `trust-task-error` carrying `expired`, where one can be returned. The document's own `expiresAt` is what expires; the DIDComm `expires_time` header is a **message** lifetime and is not it — [§2](#2-document-carriage) already makes the document's members authoritative for framework processing, and a mediator dropping a message whose `expires_time` has passed says nothing about the document's state. |

Four substitutions this transport invites, stated so no implementer has to infer them:

* **An acknowledged queue delivery is not `received`, and never `executing`.** It is the framework's own worked example, and on this transport it has a concrete shape: a mediator's `messagepickup` `messages-received`, a `delivery` acknowledgement, a status update showing the queue drained, or a `202` from a mediator's inbound endpoint. Every one of those is an acknowledgement by the **mediator**, which is not the *consumer* and does not run the [SPEC §7.2](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#72-consumer-requirements) pipeline. A message can sit acknowledged in a queue indefinitely, be redelivered, or be dropped, and the document is in `received` for none of it.
* **A closed connection is not `cancelled`.** A dropped WebSocket, a mediator that stops responding, a re-connect that loses a live session — each terminates a *delivery*. A document already accepted under [§6.1](#61-duplicate-execution-record) survives it and may still be executing. Semantic cancellation over this binding is a `trust-task-control` document and nothing else.
* **A `problem-report` is not an *error response*.** DIDComm's own problem-reporting is a transport-layer signal about a *message*. The framework's `errored` state is carried by a `trust-task-error` document, which names the reported-on document by `inResponseTo` ([SPEC §8.2](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#82-error-payload)); a `problem-report` names a DIDComm message and cannot be correlated to a document state.
* **Silence is not a state, and on this transport there is a great deal of it.** A one-way message that produces no reply is consistent with `received`, `validated`, `accepted`, `executing`, `responded` — and with the message having been dropped by a mediator, refused at the envelope layer, or never delivered. A *producer* **MUST NOT** infer any of them. Where a reply matters, [SPEC §8.4](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#84-retry-semantics)'s bit-for-bit resend is the remedy, and [§6.1](#61-duplicate-execution-record) is what makes resending safe.

Because the transport is one-way, this binding expresses **no** counterpart for `received`, `validated`, `accepted` or `executing`. Only the four states that have a reserved framework reply — `suspended`, `responded`, `errored`, `cancelled` — become observable, and each becomes observable as a *document*, never as a DIDComm event.
...
1. **The record is keyed on the *Trust Task document*'s `id`.** The DIDComm
   `@id`, `thid`, and `pthid` **MUST NOT** substitute for it, per §7.2's
   keying paragraph. This is not a formality on this transport: a mediator
   redelivery is a *new* DIDComm message carrying the *same* document, so it
   arrives with a fresh `@id` — and, where the document sets no `threadId`,
   possibly a fresh `thid` as well. A record keyed on a transport identifier
   admits every redelivery and executes the task again, which is the outcome
   item 11 exists to prevent.
