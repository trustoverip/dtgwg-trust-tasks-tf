...
What `basic-message` genuinely retained is **visible degradation in a mixed fleet**: an unaware wallet shows a harmless "Trust Task: …" line where a dedicated type is dropped silently. That is a real loss, and it is accepted here because [SPEC §10](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#10-discovery-and-capability-negotiation) capability negotiation — and RFC 0031 on the v1 side — is what gates a peer speaking Trust Tasks at all. A party that negotiated the capability and then dropped the message has a fault to surface; a party that never negotiated it should not be sent one.
...
- **Negotiate before sending.** Because an unaware peer drops an unknown type silently, a *producer* **SHOULD** establish that the peer speaks this binding — RFC 0031 discover-features, or the framework's own [§11](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#10-discovery-and-capability-negotiation) discovery — rather than inferring it from a connection existing.
...
Required by [§9.1](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#91-what-a-transport-binding-specifies), which makes the mapping a **MUST** for every binding: each state of [SPEC §4.12 *Document Lifecycle*](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#412-document-lifecycle) is named below against the v1 event that carries it, or declared to have no counterpart here.

v1 is one-way and mediated. There is no request/response pair at the transport layer, so **every** reply named below is itself a *Trust Task document* carried the same way the request was — this binding's own message type, `trust-task` attachment, over the same connection, correlated per [§3.1](#31-thread-correlation) and by `inResponseTo` ([SPEC §8.2](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#82-error-payload)). v1 has a rich decorator vocabulary of its own and none of it is a document state.

| Lifecycle state | DIDComm v1 counterpart |
|---|---|
| `received` | **No counterpart.** Unpacking the envelope, selecting the `trust-task` attachment by `@id`, and deserialising it happen inside the *consumer* and emit nothing. A mediator's pickup acknowledgement is not this state, and neither is an RFC 0015 `ack` — see below. |
...
| `accepted` | **No counterpart**, unless the *Trust Task specification* defines a reply that acknowledges acceptance. The duplicate-execution record of [§6.1](#61-duplicate-execution-record) is written internally. Where the *consumer* is understood but **blocked**, the framework's reserved `trust-task-next-step` document is carried as an ordinary message of this binding's type and the document remains `accepted` with the exchange open. |
| `executing` | **No counterpart.** Nothing is emitted when work begins. |
| `suspended` | The response to the `trust-task-control` document that suspended it ([SPEC §11](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#11-task-control)), carried the same way and correlated per [§3.1](#31-thread-correlation). v1 expresses no transport-level counterpart: a connection going idle, a mediator queue that stops draining, or a wallet that is simply not running is not suspension. |
| `responded` | A `<type>#response` document in this binding's carriage, returned over the same connection. Where the specification defines no success response, the framework's empty `{}` `#response` is the courtesy acknowledgement, sent the same way — and it is a *Trust Task document*, not a v1 `ack`. |
| `errored` | A `trust-task-error` document carried as [§4](#4-error-mapping) requires. [§4](#4-error-mapping)'s table also names the cases where **no** framework error is emitted: a decryption or signature failure, and a foreign `@type`, both stop before the pipeline with no reply at all; an unattributable authcrypt sender yields `proofRequired` only where the *consumer* has some other means of replying, and otherwise **SHOULD NOT** be answered. There the failure is unobservable to the *producer*, exactly as the framework's `errored` row anticipates. |
| `cancelled` | The response to the `trust-task-control` document, for a *producer*-requested cancellation; a `trust-task-error` carrying `cancelled` where the *consumer* stopped of its own accord. Nothing at the v1 layer expresses this — see below. |
| `expired` | A `trust-task-error` carrying `expired`, where one can be returned. The document's own `expiresAt` is what expires; the `~timing` decorator's `expires_time` is a **message** lifetime and is not it. [§2](#2-document-carriage) already requires a *consumer* to ignore decorators it does not recognize, and a mediator expiring a queued message says nothing about the document's state. |

Five substitutions this transport invites, stated so no implementer has to infer them:

* **An acknowledged pickup is not `received`, and never `executing`.** A `messagepickup` delivery acknowledgement, a batch drained, or a mediator reporting its queue empty is an acknowledgement by the **mediator**, which is not the *consumer* and does not run the [SPEC §7.2](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#72-consumer-requirements) pipeline. This is the framework's own worked example of the confusion, and mediated v1 is where it is easiest to reach for.
* **An RFC 0015 `ack` is not a framework reply.** A `~please_ack` decorator and the `ack` it elicits are v1 machinery about a *message*, emitted by the agent framework and often before any handler has seen the attachment. [§2](#2-document-carriage) makes acknowledgement requests one of the decorators a *consumer* ignores; an `ack` therefore establishes neither `received` nor `accepted`, and a *producer* **MUST NOT** read it as either.
* **A silently dropped message is not any state.** An unaware peer — including a `0.1` *consumer* meeting a `0.2` producer's `@type` ([§7.1](#71-changes-from-01)) — drops an unrecognized type without reporting anything, because an unknown `@type` is not an error condition v1 can surface. The result is indistinguishable from a document in `received`, from one still queued at a mediator, and from one that arrived and is executing. Capability negotiation before sending ([§2.2](#22-implementation-notes-aries-frameworks)) is what avoids this, not a lifecycle inference after the fact.
* **A `problem-report` is not an *error response*.** v1 problem-reporting is a transport-layer signal about a message. The `errored` state is carried by a `trust-task-error` document, which names the reported-on document by `inResponseTo`; a `problem-report` names a v1 message and cannot be correlated to a document state.
* **A closed connection is not `cancelled`.** A dropped socket, a mediator that stops responding, a wallet that goes offline — each terminates a *delivery*. A document already accepted under [§6.1](#61-duplicate-execution-record) survives it and may still be executing. Semantic cancellation over this binding is a `trust-task-control` document and nothing else.

Because the transport is one-way, this binding expresses **no** counterpart for `received`, `validated`, `accepted` or `executing`. Only the four states that have a reserved framework reply — `suspended`, `responded`, `errored`, `cancelled` — become observable, and each becomes observable as a *document* in this binding's own carriage, never as a v1 decorator or protocol message.
