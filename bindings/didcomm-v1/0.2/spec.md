...
| `suspended` | The response to the `trust-task-control` document that suspended it ([SPEC §12](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#12-task-control)), carried the same way and correlated per [§3.1](#31-thread-correlation). v1 expresses no transport-level counterpart: a connection going idle, a mediator queue that stops draining, or a wallet that is simply not running is not suspension. |
