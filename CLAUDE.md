...
| `npm run check-dart-packages` | the five files naming the Dart packages (`publish.yml`'s `tag-dart` matrix, `release-pr.sh`'s `DART_PACKAGES`, `publish-dart.yml`, `release-bump-dart.sh`, `dart.yml`) name the same packages with the same dirs and tag prefixes | whether a package's first manual publish and trusted-publishing setup happened |
...
than the registry. Its `release-pr` job (`scripts/release-pr.sh`) keeps **one**
Release PR, on the `release` branch, up to date with the version bumps and
changelog entries the merged commits imply for **every** package — crates via
`release-plz update`, npm/Go/Dart via `scripts/release-bump-{ts,go,dart}.sh`,
because release-plz is Rust-only and cannot bump a `package.json` or a
`pubspec.yaml`, and the Go module has no manifest at all. Merging *that* PR is
the release. (It used to be one Release PR per package — up to thirteen, each
regenerated by every merge of another. A new publishable package now needs an
entry in `release-pr.sh`'s package lists as well as its publish matrix leg.)
...
4. **Go from v2 onward needs the major in the module path.** `trust-tasks-go/v2`
   ...
   `scripts/release-bump-go.sh` refuses to propose such a bump and tells you what to
...
- **Released on its own `trust-tasks-go/proof/vX.Y.Z` tag**, independently of the
  ...
  as the core's does), a matrix leg in `publish-go`, and a `GO_MODULES` entry in
  `scripts/release-pr.sh` (`release-bump-go.sh trust-tasks-go/proof`). The core's release WATCH **excludes**
...
- **Core CI does not reach it**; a `didcomm` job in `go.yml` builds/vets/tests on
  ...
  `GO_MODULES` entry in `release-pr.sh` — independent of the core. **Cross-library interop is now
...
- **Release wiring is per package.** Each npm package has a `publish-npm`
  matrix leg and a `TS_PACKAGES` entry in `scripts/release-pr.sh`; its first
  npm publish is manual (RELEASING.md).
...
3. **Two version declarations must move together** — `version:` in
   ...
   `release-bump-dart.sh` did exactly that, because its regex could not match a
...
- **It cannot be published yet.** `affinidi_tsp` is not on pub.dev, so
  ...
  lists** (`tag-dart` in `publish.yml`, `DART_PACKAGES` in `release-pr.sh`), its tag pattern
  and case arm in `publish-dart.yml`, and the `release-bump-dart.sh` case — a
...
- **Like `trust_tasks_tsp` it is tested but not released yet** — it is in the
  ...
  (`tag-dart`, `DART_PACKAGES`), `publish-dart.yml` and `release-bump-dart.sh`,
