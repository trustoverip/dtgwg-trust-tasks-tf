...
For framework-level contributions: the framework text is maintained in [trustoverip/dtgwg-trust-tasks-spec](https://github.com/trustoverip/dtgwg-trust-tasks-spec), and `SPEC.md` here is generated from it (`npm run generate-framework-spec`) — do not edit it. For the build pipeline or the website, see [`CONTRIBUTING.md`](CONTRIBUTING.md).
...
- Publishing a slug in the reserved namespace means editing **two** hand-maintained places: the reserved-slug table of SPEC §6.1 — in the [canonical framework repository](https://github.com/trustoverip/dtgwg-trust-tasks-spec), since `SPEC.md` here is generated from it — and the allowlist at `specs/spec.meta.schema.json` → `#/properties/slug/anyOf[1]/enum`. Miss the second and the build reports `/slug must NOT be valid`, which is the schema's way of saying "you hit the reservation and are not on the list". `npm run new-spec` refuses reserved slugs outright rather than emitting something that cannot validate.
...
issuedAtRequirement:               # SPEC §7.3 item 17 — REQUIRED for a
  requirement: REQUIRED            # consequential Trust Task; see below
  rationale: A one-sentence reason this task's documents must be placeable in time.
...
- **`issuedAtRequirement` is how you say the thing [SPEC §7.3 item 17](SPEC.md#73-specification-requirements) makes you say.** It is shaped exactly like `proofRequirement` — a single `requirement`, or a per-variant `request` / `response` pair, plus a `rationale` — because it does the same job for a different envelope member. The framework's baseline for `issuedAt` is a **SHOULD** ([§4.2](SPEC.md#42-top-level-members)); declaring `REQUIRED` raises it to a **MUST** for documents conforming to your spec, emits `Payload::IS_ISSUED_AT_REQUIRED` / `isIssuedAtRequired` in both generated libraries, and makes every conforming consumer reject a document with no `issuedAt` as `malformedRequest`. (`malformedRequest` and not `expired`: [§8.3](SPEC.md#83-standard-error-codes) defines no dedicated code, and `expired` names a document that was once acceptable, which one that could never be placed in a window never was. It is the same code [§7.2 item 13](SPEC.md#72-consumer-requirements) already uses for the other freshness rejections.)

  **If your task is *consequential*, item 17 says you MUST declare `REQUIRED`.** A task is consequential ([SPEC §3](SPEC.md#3-terminology)) when `sideEffects.level` is `mutating` or `destructive`, or `exposure.discloses` is `secret`, or `exposure.actsAsSubject` is `true` — so you can read the answer off declarations you have already made. The reason is [§7.2 item 11](SPEC.md#72-consumer-requirements): duplicate-execution protection is implementable only over a bounded window, and a document carrying neither `expiresAt` nor `issuedAt` gives its consumer no window to place it in — leaving that consumer to retain its record forever or refuse to execute your task at all.

  **The build derives that floor and reports it; it does not declare it for you.** `npm run validate` prints a `Freshness floor (§7.3 item 17)` line, and **fails** both on a consequential spec that declares nothing and on one that declares something weaker than `REQUIRED`. Deriving the value outright was considered and rejected: `sideEffects` and `exposure` are **descriptive**, and correcting `sideEffects.level` from `none` to `mutating` for accuracy must not silently change which documents every consumer of your spec is obliged to reject. Declaring it keeps the requirement visible in the front matter a reader scans, and keeps a change to it visible in review. The undeclared case was a counted warning only while the 177 live consequential drafts that predate the member caught up; the whole registry now declares it, so there is no opt-out flag. **A `retired` spec is exempt** — [§5.3](SPEC.md#53-maturity-levels) freezes its schema and prose, and adding a declaration would change what consumers reject for a specification that can no longer take the `MAJOR` increment [§5.2](SPEC.md#52-compatibility-rules) requires. Those are reported separately as frozen, alongside the error-code lints' frozen counts.
...
The field is rendered as the **Implementations** row of the spec page's Metadata grid (and carried in `registry.json`), showing `Not declared` when the list is absent.
