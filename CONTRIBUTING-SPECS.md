...
- **`issuedAtRequirement` is how you say the thing [SPEC §7.3 item 17](SPEC.md#73-specification-requirements) makes you say.** It is shaped exactly like `proofRequirement` — a single `requirement`, or a per-variant `request` / `response` pair, plus a `rationale` — because it does the same job for a different envelope member. The framework's baseline for `issuedAt` is a **SHOULD** ([§4.2](SPEC.md#42-top-level-members)); declaring `REQUIRED` raises it to a **MUST** for documents conforming to your spec, emits `Payload::IS_ISSUED_AT_REQUIRED` / `isIssuedAtRequired` in both generated libraries, and makes every conforming consumer reject a document with no `issuedAt` as `malformedRequest`. (`malformedRequest` and not `expired`: [§8.3](SPEC.md#83-standard-error-codes) defines no dedicated code, and `expired` names a document that was once acceptable, which one that could never be placed in a window never was. It is the same code [§7.2 item 13](SPEC.md#72-consumer-requirements) already uses for the other freshness rejections.)
  ...
  **If your task is *consequential*, item 17 says you MUST declare `REQUIRED`.** A task is consequential ([SPEC §2](SPEC.md#2-terminology)) when `sideEffects.level` is `mutating` or `destructive`, or `exposure.discloses` is `secret`, or `exposure.actsAsSubject` is `true` — so you can read the answer off declarations you have already made. The reason is [§7.2 item 11](SPEC.md#72-consumer-requirements): duplicate-execution protection is implementable only over a bounded window, and a document carrying neither `expiresAt` nor `issuedAt` gives its consumer no window to place it in — leaving that consumer to retain its record forever or refuse to execute your task at all.
