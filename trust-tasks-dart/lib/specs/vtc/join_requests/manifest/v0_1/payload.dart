// Code generated by scripts/build-dart-bindings.mjs. DO NOT EDIT.
// Source: specs/vtc/join-requests/manifest/0.1/payload.schema.json

import 'package:trust_tasks/trust_tasks.dart';

/// Vendor-namespaced extension object per SPEC.md §4.5.1. Each immediate key MUST be a
/// reverse-DNS namespace; structure under each namespace is opaque to the framework.
typedef Ext = Map<String, dynamic>;

/// ResponseCriteriaItem, generated from its schema.
class ResponseCriteriaItem {
  const ResponseCriteriaItem({
    required this.id,
    this.description,
    required this.presentationDefinition,
  });

  /// Read this payload from a decoded JSON object.
  factory ResponseCriteriaItem.fromJson(Map<String, dynamic> json) =>
      ResponseCriteriaItem(
        id: json['id'] as String,
        description: json['description'] as String?,
        presentationDefinition:
            json['presentationDefinition'] as Map<String, dynamic>,
      );

  final String id;
  final String? description;

  /// The presentation-definition an applicant must satisfy for this criterion (opaque
  /// here).
  final Map<String, dynamic> presentationDefinition;

  /// Serialize to a JSON-encodable map, omitting absent members.
  Map<String, dynamic> toJson() => <String, dynamic>{
        'id': id,
        if (description != null) 'description': description!,
        'presentationDefinition': presentationDefinition,
      };
}

/// VTC Join-Requests Manifest — response payload
class Response {
  const Response({
    required this.communityDid,
    required this.criteria,
    this.ext,
  });

  /// Read this payload from a decoded JSON object.
  factory Response.fromJson(Map<String, dynamic> json) => Response(
        communityDid: json['communityDid'] as String,
        criteria: (json['criteria'] as List<dynamic>)
            .map(
                (e) => ResponseCriteriaItem.fromJson(e as Map<String, dynamic>))
            .toList(),
        ext: json['ext'] as Map<String, dynamic>?,
      );

  final String communityDid;
  final List<ResponseCriteriaItem> criteria;
  final Ext? ext;

  /// Serialize to a JSON-encodable map, omitting absent members.
  Map<String, dynamic> toJson() => <String, dynamic>{
        'communityDid': communityDid,
        'criteria': criteria.map((e) => e.toJson()).toList(),
        if (ext != null) 'ext': ext!,
      };
}

/// VTC Join-Requests Manifest — payload
class Payload {
  const Payload({
    this.ext,
  });

  /// Read this payload from a decoded JSON object.
  factory Payload.fromJson(Map<String, dynamic> json) => Payload(
        ext: json['ext'] as Map<String, dynamic>?,
      );

  final Ext? ext;

  /// Serialize to a JSON-encodable map, omitting absent members.
  Map<String, dynamic> toJson() => <String, dynamic>{
        if (ext != null) 'ext': ext!,
      };
}

/// The Trust Task type URI this library's [Payload] is carried under.
const String typeUri =
    'https://trusttasks.org/spec/vtc/join-requests/manifest/0.1';

/// The success-response form of [typeUri] (SPEC §4.4.1).
const String responseTypeUri =
    'https://trusttasks.org/spec/vtc/join-requests/manifest/0.1#response';

/// This specification's payload schema, as JSON text.
///
/// SPEC §7.2 item 2 is performed against this. It ships with the library rather
/// than only as a file under specs/ because a Dart class carries none of the
/// schema's constraints — not minLength, not pattern, not the oneOf mutual
/// exclusion — so without it every such rule is unenforced. Cross-file \$refs are
/// already inlined, so it needs no resolver.
const String payloadSchemaJson =
    '{"\$schema":"https://json-schema.org/draft/2020-12/schema","\$id":"https://trusttasks.org/spec/vtc/join-requests/manifest/0.1","title":"VTC Join-Requests Manifest — payload","type":"object","additionalProperties":false,"properties":{"ext":{"\$ref":"#/\$defs/Ext"}},"\$defs":{"Response":{"\$anchor":"response","title":"VTC Join-Requests Manifest — response payload","type":"object","additionalProperties":false,"required":["communityDid","criteria"],"properties":{"communityDid":{"type":"string","minLength":1},"criteria":{"type":"array","items":{"type":"object","additionalProperties":false,"required":["id","presentationDefinition"],"properties":{"id":{"type":"string","minLength":1},"description":{"type":"string","maxLength":1024},"presentationDefinition":{"type":"object","description":"The presentation-definition an applicant must satisfy for this criterion (opaque here)."}}}},"ext":{"\$ref":"#/\$defs/Ext"}}},"Ext":{"title":"Ext","description":"Vendor-namespaced extension object per SPEC.md §4.5.1. Each immediate key MUST be a reverse-DNS namespace; structure under each namespace is opaque to the framework.","type":"object","minProperties":1,"additionalProperties":true,"propertyNames":{"pattern":"^[a-z][a-z0-9-]*(\\\\.[a-z0-9-]+)+\$"}}}}';

/// As [payloadSchemaJson], for the success-response variant.
const String responsePayloadSchemaJson =
    '{"\$schema":"https://json-schema.org/draft/2020-12/schema","\$ref":"#/\$defs/Response","\$defs":{"Response":{"\$anchor":"response","title":"VTC Join-Requests Manifest — response payload","type":"object","additionalProperties":false,"required":["communityDid","criteria"],"properties":{"communityDid":{"type":"string","minLength":1},"criteria":{"type":"array","items":{"type":"object","additionalProperties":false,"required":["id","presentationDefinition"],"properties":{"id":{"type":"string","minLength":1},"description":{"type":"string","maxLength":1024},"presentationDefinition":{"type":"object","description":"The presentation-definition an applicant must satisfy for this criterion (opaque here)."}}}},"ext":{"\$ref":"#/\$defs/Ext"}}},"Ext":{"title":"Ext","description":"Vendor-namespaced extension object per SPEC.md §4.5.1. Each immediate key MUST be a reverse-DNS namespace; structure under each namespace is opaque to the framework.","type":"object","minProperties":1,"additionalProperties":true,"propertyNames":{"pattern":"^[a-z][a-z0-9-]*(\\\\.[a-z0-9-]+)+\$"}}}}';

/// The SPEC §7.2 policy for the request variant, taken from this
/// specification's front matter.
///
/// Pass it to [consumeInbound] — items 5b, 7 and 8 are per-specification and
/// cannot be derived from the document alone, and item 2 needs the schema it
/// carries.
const SpecPolicy spec = SpecPolicy(
  typeUri: typeUri,
  isBearer: false,
  isProofRequired: false,
  isRecipientRequired: true,
  isIssuedAtRequired: false,
  payloadSchema: payloadSchemaJson,
);

/// The SPEC §7.2 policy for the success-response variant.
///
/// `isRecipientRequired` tracks the *issuer* party's requirement, because a
/// response swaps the parties (§7.3 item 5).
const SpecPolicy responseSpec = SpecPolicy(
  typeUri: responseTypeUri,
  isBearer: false,
  isProofRequired: false,
  isRecipientRequired: true,
  isIssuedAtRequired: false,
  payloadSchema: responsePayloadSchemaJson,
);
