...
extension type const ResponseStatus(String value) {
  ...
  static const ResponseStatus deferred = ResponseStatus('deferred');
...
class Response {
  ...
  /// Elaboration in prose, when the decider gave one. Present only when `status` is
  /// `rejected`.
  final String? reason;

  /// When the refusal was decided — not when this poll was produced. Present only when
  /// `status` is `rejected`.
...
/// VTC Join-Requests Status — payload
class Payload {
  const Payload({
    this.requestId,
    this.ext,
  ...
  /// The request to poll. Optional: an applicant whose first reply was lost never
  /// received an id, and the id-less poll — resolved from the authenticated applicant's
