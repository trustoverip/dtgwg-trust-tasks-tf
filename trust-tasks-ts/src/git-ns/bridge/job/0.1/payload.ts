...
/** Trust Task type URI. */
export const TYPE_URI = "https://trusttasks.org/spec/git-ns/bridge/job/0.1" as const;

/** Stable alias for this specification's request payload shape. */
export type Payload = GitNamespacesBridgeJobPayload;

/** Trust Task response type URI (request type URI + "#response"). */
export const RESPONSE_TYPE_URI = "https://trusttasks.org/spec/git-ns/bridge/job/0.1#response" as const;
...
export const PAYLOAD_SCHEMA = {
  ...
  "$id": "https://trusttasks.org/spec/git-ns/bridge/job/0.1",
  ...
  "properties": {
    ...
    "kind": {
      ...
      "description": "What to do. `projectRoles` — converge forge roles to `desiredRoles`. `createRepo` — create `repo`, run the bootstrap plan, and project `desiredRoles`. `bootstrap` — run the bootstrap plan, or the listed `steps` of it, on an existing `repo`. `archive` — archive `repo`. `inspect` — compare the forge with the projection for `repo`, or for the whole namespace. `beginBind` — start the proof that the binding admin controls `target`. `beginAccountLink` — start linking `subject`'s account on this namespace's forge."
    },
    "repo": {
