/**
 * Generated by scripts/build-ts-bindings.mjs — DO NOT EDIT BY HAND.
 * Source: specs/git-ns/namespace/unbind/0.1/payload.schema.json
 */

import type { Ext, NamespaceId } from "../../../../_shared/components.js";


/**
 * A namespace admin ends the VTC's governance of a namespace. Every right recorded on a resource inside it is revoked and withdrawn from the Trust Registry, and every repository in it becomes `detached`.
 */
export interface GitNamespacesUnbindNamespacePayload {
  /**
   * The namespace to unbind.
   */
  namespace: NamespaceId;
  ext?: Ext;
}
/**
 * What unbinding removed.
 */
export interface GitNamespacesUnbindNamespaceResponsePayload {
  namespace: NamespaceId;
  /**
   * How many recorded rights were revoked.
   */
  rightsRevoked: number;
  /**
   * How many repositories became `detached`.
   */
  reposDetached: number;
  ext?: Ext;
}

/** Shared definitions this specification references, re-exported under the names it used to declare them with. */
export type { Ext, NamespaceId };

/** Trust Task type URI. */
export const TYPE_URI = "https://trusttasks.org/spec/git-ns/namespace/unbind/0.1" as const;

/** Stable alias for this specification's request payload shape. */
export type Payload = GitNamespacesUnbindNamespacePayload;

/** Trust Task response type URI (request type URI + "#response"). */
export const RESPONSE_TYPE_URI = "https://trusttasks.org/spec/git-ns/namespace/unbind/0.1#response" as const;

/** Stable alias for this specification's success-response payload shape. */
export type Response = GitNamespacesUnbindNamespaceResponsePayload;

/**
 * This specification's payload schema, as a value.
 *
 * SPEC.md §7.2 item 2 is performed against this. It is shipped as data
 * rather than only as a `.json` file because TypeScript types are erased
 * at runtime: without a schema a consumer has nothing to validate, and
 * every REQUIRED payload member is optional in practice. Cross-file
 * `$ref`s are already inlined, so it needs no resolver.
 */
export const PAYLOAD_SCHEMA = {
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://trusttasks.org/spec/git-ns/namespace/unbind/0.1",
  "title": "Git Namespaces — Unbind Namespace — payload",
  "description": "A namespace admin ends the VTC's governance of a namespace. Every right recorded on a resource inside it is revoked and withdrawn from the Trust Registry, and every repository in it becomes `detached`.",
  "type": "object",
  "additionalProperties": false,
  "required": [
    "namespace"
  ],
  "properties": {
    "namespace": {
      "$ref": "#/$defs/NamespaceId",
      "description": "The namespace to unbind."
    },
    "ext": {
      "$ref": "#/$defs/Ext"
    }
  },
  "$defs": {
    "Response": {
      "$anchor": "response",
      "title": "Git Namespaces — Unbind Namespace — response payload",
      "description": "What unbinding removed.",
      "type": "object",
      "additionalProperties": false,
      "required": [
        "namespace",
        "rightsRevoked",
        "reposDetached"
      ],
      "properties": {
        "namespace": {
          "$ref": "#/$defs/NamespaceId"
        },
        "rightsRevoked": {
          "type": "integer",
          "minimum": 0,
          "description": "How many recorded rights were revoked."
        },
        "reposDetached": {
          "type": "integer",
          "minimum": 0,
          "description": "How many repositories became `detached`."
        },
        "ext": {
          "$ref": "#/$defs/Ext"
        }
      }
    },
    "Ext": {
      "title": "Ext",
      "description": "Vendor-namespaced extension object per SPEC.md §4.5.1. Each immediate key MUST be a reverse-DNS namespace; structure under each namespace is opaque to the framework.",
      "type": "object",
      "minProperties": 1,
      "additionalProperties": true,
      "propertyNames": {
        "pattern": "^[a-z][a-z0-9-]*(\\.[a-z0-9-]+)+$"
      }
    },
    "NamespaceId": {
      "title": "NamespaceId",
      "description": "The VTC's opaque identifier for a namespace, assigned when it is bound. Stable for the life of the binding; never reused for another binding.",
      "type": "string",
      "minLength": 1,
      "maxLength": 128
    }
  }
} as const;

/** As {@link PAYLOAD_SCHEMA}, for the success-response variant. */
export const RESPONSE_PAYLOAD_SCHEMA = {
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$ref": "#/$defs/Response",
  "$defs": {
    "Response": {
      "$anchor": "response",
      "title": "Git Namespaces — Unbind Namespace — response payload",
      "description": "What unbinding removed.",
      "type": "object",
      "additionalProperties": false,
      "required": [
        "namespace",
        "rightsRevoked",
        "reposDetached"
      ],
      "properties": {
        "namespace": {
          "$ref": "#/$defs/NamespaceId"
        },
        "rightsRevoked": {
          "type": "integer",
          "minimum": 0,
          "description": "How many recorded rights were revoked."
        },
        "reposDetached": {
          "type": "integer",
          "minimum": 0,
          "description": "How many repositories became `detached`."
        },
        "ext": {
          "$ref": "#/$defs/Ext"
        }
      }
    },
    "Ext": {
      "title": "Ext",
      "description": "Vendor-namespaced extension object per SPEC.md §4.5.1. Each immediate key MUST be a reverse-DNS namespace; structure under each namespace is opaque to the framework.",
      "type": "object",
      "minProperties": 1,
      "additionalProperties": true,
      "propertyNames": {
        "pattern": "^[a-z][a-z0-9-]*(\\.[a-z0-9-]+)+$"
      }
    },
    "NamespaceId": {
      "title": "NamespaceId",
      "description": "The VTC's opaque identifier for a namespace, assigned when it is bound. Stable for the life of the binding; never reused for another binding.",
      "type": "string",
      "minLength": 1,
      "maxLength": 128
    }
  }
} as const;

/**
 * SPEC.md §7.2 policy for the request variant, from this specification's
 * front matter. Pass to `consumeInbound` — items 5b, 7 and 8 are
 * per-specification and cannot be derived from the document alone, and
 * item 2 needs the schema this carries.
 */
export const SPEC = {
  typeUri: TYPE_URI,
  isBearer: false,
  isProofRequired: true,
  isRecipientRequired: true,
  isIssuedAtRequired: true,
  payloadSchema: PAYLOAD_SCHEMA,
} as const;

/**
 * SPEC.md §7.2 policy for the success-response variant. `isRecipientRequired`
 * tracks the *issuer* party's requirement because a response swaps the
 * parties (§7.3 item 5).
 */
export const RESPONSE_SPEC = {
  typeUri: RESPONSE_TYPE_URI,
  isBearer: false,
  isProofRequired: true,
  isRecipientRequired: true,
  isIssuedAtRequired: true,
  payloadSchema: RESPONSE_PAYLOAD_SCHEMA,
} as const;
