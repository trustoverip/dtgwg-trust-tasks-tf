...
1.1 finalizes bundles uploaded by the `chunkedTrustTask` algorithm as well as by `stream` (see [Chunked transfer](../../initiate-export/1.1/spec.md#chunked-transfer)). It adds the section [Before the password is used](#before-the-password-is-used) and two error codes, `incompleteUpload` and `bundleDigestMismatch`. The payload schema is unchanged apart from its `$id`; the new codes are the reason for a new version, since a 1.0 producer has no way to act on them. That is an additive change and a `MINOR` increment under [SPEC §5.2](/SPEC.md#52-compatibility-rules).
...
**`bundleId`** — the handle returned in the descriptor from [`initiate-import`](../../initiate-import/1.1/spec.md). Opaque: a producer quotes what it was given and **MUST NOT** derive, guess or enumerate one.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vta/backup/finalize-import/1.1#request",
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vta/backup/finalize-import/1.1#response",
