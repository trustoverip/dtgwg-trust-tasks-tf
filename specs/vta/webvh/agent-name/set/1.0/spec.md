...
exposure:
  ...
  actsAsSubject: false
errorCodes:
  - code: "vta/webvh/agent-name/set:notFound"
...
**VTA WebVH Agent-Name — Set** binds an agent name to a DID.
...
A conforming **consumer** (the VTA) **MUST NOT** represent the binding as
verified. Recording it here is the VTA's half; the DID document's `alsoKnownAs`
is the other, and until both exist a resolver has no basis to trust the name.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vta/webvh/agent-name/set/1.0",
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vta/webvh/agent-name/set/1.0#response",
...
A name recorded but never claimed back in the DID document is a name that
resolves without proving anything. Anyone reading the redirect learns where it
points; nobody learns that the DID agreed.
