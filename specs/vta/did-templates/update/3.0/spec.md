...
issuedAtRequirement:
  requirement: REQUIRED
  rationale: An update overwrites the template, so a stale copy restores an earlier DID shape and every identifier created afterwards inherits it.
sideEffects:
  level: mutating
...
* **DidTemplate / DidTemplateRecord.** The authored and persisted template shapes — see the [shared definition](../../../_shared/0.2/did-template.schema.json).
