...
proofRequirement:
  requirement: REQUIRED
  rationale: >-
    Abort destroys staged bytes and, on the export side, closes the only window in which a copy of the node could have been retrieved. A recipient that cannot attribute the request cannot answer the question an operator asks when a bundle they were about to download has vanished: who cancelled it.
issuedAtRequirement:
  requirement: REQUIRED
  rationale: >-
...
The producer is the operator that initiated the bundle; the recipient is the node holding it. The request payload is the top-level schema in [`payload.schema.json`](payload.schema.json).
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/backup/abort/0.1#request",
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/backup/abort/0.1#response",
