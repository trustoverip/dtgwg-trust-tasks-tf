---
slug: backup/initiate-import
version: "0.1"
title: "Backup — Initiate Import"
summary: "Mint an upload slot for an encrypted bundle and return the descriptor that writes to it, over HTTPS or chunk by chunk over the node's own Trust Task transport."
status: draft
...
proofRequirement:
  ...
  response: REQUIRED
  rationale: >-
    The request opens a writable slot in the recipient and pre-commits what will be written to it, so an unattributable one is an anonymous party arranging to place bytes inside a node. The response is REQUIRED for a different reason: a `stream` descriptor carries a bearer token for that slot, and an unsigned descriptor is one an intermediary can substitute, redirecting the operator's upload to a destination the node never minted.
issuedAtRequirement:
  requirement: REQUIRED
  rationale: >-
    A replayed initiate-import opens a second write slot from one authorization. Each slot is a place bytes may be written inside the node, so bounding replay is what keeps the number of open doors equal to the number of times a human asked for one.
sideEffects:
  level: mutating
  rationale: >-
    Reserves a bundle slot and, for `stream`, mints a short-lived, write-capable transport endpoint. Nothing the node holds is read, altered or destroyed — the uploaded bytes are staged and inert until finalize-import applies them, and a slot that is never finalized expires having changed nothing.
exposure:
  discloses: secret
  ingests: metadata
  actsAsSubject: false
  rationale: >-
    A `stream` response carries `transportToken`, a bearer credential that authorizes writing bytes into the recipient. It is classed secret for the same reason the export descriptor's token is, with the direction reversed: leaking an export token exposes the node's state, and leaking an import token lets someone else fill the slot the operator opened. Inbound the request carries only digests, byte counts and an algorithm name — descriptive metadata about bytes the recipient does not yet hold.
retention:
  class: exchange
  rationale: >-
...
errorCodes:
  ...
  - code: backup/initiate-import:invalidDigest
    ...
    retryable: false
  - code: backup/initiate-import:invalidManifest
    meaning: >-
    ...
    retryable: false
  - code: backup/initiate-import:chunkSizeUnacceptable
    meaning: >-
    ...
    detailsSchema:
      ...
      properties:
        maxChunkSize:
          ...
          minimum: 16384
          maximum: 262144
  - code: backup/initiate-import:transportUnavailable
    meaning: >-
    ...
    retryable: false
  - code: backup/initiate-import:tooManyOpenBundles
    meaning: >-
...
The **Backup — Initiate Import** Trust Task asks a node to open a slot for an encrypted bundle the operator is about to upload, and to return the terms on which those bytes are to be written.

It is the mirror of [`initiate-export`](../../initiate-export/0.1/spec.md), and the asymmetry is worth naming: export mints something readable, import mints something **writable**. That makes this the task in the family with the smallest payload and the largest consequence, because what it opens is a door into the node rather than a window onto it.

Nothing is applied here. The uploaded bytes are staged, inert, and remain so until [`finalize-import`](../../finalize-import/0.1/spec.md) supplies the password that decrypts them and commits the result. A slot that is never finalized expires having changed nothing about the node — which is what lets an operator upload first and decide afterwards.
...
The authority is **custody of the node itself**, on the same terms as export and for a symmetric reason. An import does not write to a resource inside the node that a capability could name; it stages the material from which the node's entire state — including the key material every other authority derives from — will be replaced.

A recipient **MUST** refuse a producer holding anything less, and **MUST NOT** derive import authority from any accumulation of narrower write grants. A producer entitled to add an entry through this node is not thereby entitled to supply the node a new set of everything.
...
This specification does not, and **MUST NOT**, declare that a human approval or a step-up is required ([SPEC §7.3 item 13](/SPEC.md#73-specification-requirements)). A consumer choosing to place one **SHOULD** be aware that this task is the weaker of the two places to do it: nothing is applied here, and the document that actually replaces the node's state is `finalize-import`. An approval attached only to the slot leaves the commitment ungated.
...
**`descriptor`** — the response's account of where or how to write, and until when, in one of the two shapes defined by the shared [`BundleDescriptor`](../../../vta/_shared/0.1/backup-transfer.schema.json). A recipient **MUST** return the algorithm the request named, or refuse. Its members are the same `initiate-export` returns, read in the opposite direction:
...
The algorithm is defined once, under [Chunked transfer](../../initiate-export/0.1/spec.md#chunked-transfer) in `initiate-export/0.1`; the manifest rules, the 262144-byte chunk-size bound and its derivation, and the expiry rules apply here unchanged. What differs in this direction:
...
The producer is the backup operator; the recipient is the node that will receive the bundle. The request payload is the top-level schema in [`payload.schema.json`](payload.schema.json).
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/backup/initiate-import/0.1#request",
...
A 524300-byte bundle divided into 256 KiB chunks, for a node reachable only over DIDComm or TSP.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/backup/initiate-import/0.1#response",
  ...
  "payload": {
    "descriptor": {
      ...
      "algorithm": "stream",
      "transportUrl": "https://node.example/backup/blob/9c858901-8a57-4791-81fe-4c455b099bc9",
...
The recipient learns when this operator imports and how often, which describes an operational rhythm: imports cluster around migrations, recoveries and incidents in a way exports do not. That is intrinsic — a node cannot accept a bundle without knowing it did.

`expectedSha256` is a stable identifier for the bundle's exact bytes, and each entry of `chunkDigests` is a stable identifier for one slice of them. Two nodes given the same bundle can, if they compare notes, establish that they received the same one; so can an intermediary that sees both requests. That is unavoidable for values whose purpose is to be compared, and it is worth noting that the digests are of the *encrypted* bundle, so they identify the copy rather than the contents.
...
The purpose is recovery: staging a previously exported bundle so the node can be restored from it. The material a producer uploads is a complete node, so a recipient **MUST NOT** treat a staged bundle as a source of data for anything other than the `finalize-import` that names it — in particular, it **MUST NOT** be inspected, indexed, or partially applied, and a partially uploaded chunked bundle is not a partial node.
