---
slug: backup/put-chunk
version: "0.1"
title: "Backup — Put Chunk"
...
sideEffects:
  level: mutating
  rationale: >-
    Stages one chunk of an import bundle. Nothing the node holds is read, altered or destroyed; staged chunks are inert until finalize-import applies the assembled bundle, and a slot that is never finalized expires having changed nothing.
exposure:
  discloses: metadata
  ingests: secret
  actsAsSubject: false
  rationale: >-
    Inbound, each request carries part of an encrypted, complete copy of a node — classed with the whole bundle, because the chunks together are the bundle and the bundle is protected only by its password. Outbound, the response is an acknowledgement and a count, and says nothing about the chunk's content.
retention:
  class: exchange
  rationale: >-
...
The **Backup — Put Chunk** Trust Task writes one chunk of an encrypted bundle into an import slot opened by [`initiate-import`](../../initiate-import/0.1/spec.md) with the `chunkedTrustTask` algorithm.

It is the import-side counterpart of [`get-chunk`](../../get-chunk/0.1/spec.md), for the same reason: a node reachable only over DIDComm or TSP cannot accept a whole bundle in one message, but can accept it a bounded chunk at a time. The algorithm is defined once, under [Chunked transfer](../../initiate-export/0.1/spec.md#chunked-transfer).

In this direction the operator has the bytes, so the operator sends them — but every write is individually answered, and a chunk is not held until its response says it is. A send the transport accepts is not a chunk the node stored, and the answer is what turns one into the other.

Every chunk is checked against the manifest the operator committed when the slot was opened, before the node held any of it. A write cannot introduce bytes the operator did not commit to, and cannot change a chunk already stored.
...
The entitlement is narrow because the decision that matters was already taken: `initiate-import` demanded custody of the node before it opened the slot, and `finalize-import` demands it again before anything is applied. A write in between stages inert bytes that must match a commitment the creator made.
...
The producer is the operator that opened the slot; the recipient is the node holding it. The request payload is the top-level schema in [`payload.schema.json`](payload.schema.json).
...
To intermediaries, a chunked upload is a run of `chunkCount` large, similarly sized requests from one operator to one node, each followed by a small response. A mediator that cannot read them still learns that a large transfer of that shape happened, and roughly how large. That is intrinsic to moving a bundle over a mediated transport.
...
Four conditions are answered as `notFound`: no slot with that id, an export bundle, a `stream` slot, and a slot owned by another operator. Conflating them keeps an opaque handle from becoming an oracle over the node's backup activity. The other codes are returned only to the slot's creator.
