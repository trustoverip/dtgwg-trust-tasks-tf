...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/auth/passkey/enroll/finish/0.2`, with itself as `issuer` and the auth service as `recipient`.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/auth/passkey/enroll/finish/0.2",
...
A success *response* document carries `type: https://trusttasks.org/spec/auth/passkey/enroll/finish/0.2#response`. Payload: `{ credentialId, subject, deviceLabel?, registeredAt }`.
