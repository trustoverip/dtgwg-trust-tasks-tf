...
**Added `uvOptions` to the response** (optional), and its counterpart `uvCredential` to [`enroll/finish/0.2`](../../finish/0.2/spec.md). The framework target moves to 0.2.

0.1 modelled enrollment as a single registration ceremony authorized by the `proof`. That is right for the *first* passkey, where the proof is the only credential the subject has. It is not right for a subsequent one, because adding an authenticator to an account that already has one is a privilege escalation available to anyone holding a token: enroll silently, and you have durable access that survives the subject changing everything else.

0.2 lets the consumer demand that an **existing** authenticator authorize the addition, in the same round-trip. `uvOptions` is optional precisely because the first enrollment cannot satisfy it — there is nothing enrolled to verify against.

**Compatibility.** A 0.1 producer talking to a 0.2 consumer that issues `uvOptions` will fail at finish, since it cannot return an assertion it never obtained. That is the intended outcome: the alternative is a consumer silently dropping a control it decided it needed. Consumers introducing re-authentication **SHOULD** announce it before enabling it.
...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/auth/passkey/enroll/start/0.2`, with itself as `issuer` and the auth service as `recipient`.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/auth/passkey/enroll/start/0.2",
...
A success *response* document carries `type: https://trusttasks.org/spec/auth/passkey/enroll/start/0.2#response`. Payload: `{ enrollmentId, options }`, plus `uvOptions` when the consumer requires re-authentication.
...
### Adding a second device, with re-authentication

The same response, for a subject who already has a passkey. `options` is unchanged; `uvOptions` is the added half, and its `allowCredentials` lists what the subject already holds.
...
The two challenges are distinct and both bound to the `enrollmentId`. Reusing one value for both would let a single ceremony's output satisfy the other check.

## Security & Privacy

**Re-authentication closes the silent-enrollment path.** Without it, a captured token is enough to bind an attacker-controlled authenticator to the subject's VID — access that outlives the token, survives a password change, and looks exactly like a legitimate device in [`auth/passkey/list`](../../../list/0.1/spec.md). Requiring an existing authenticator to authorize the addition means a stolen token alone cannot grow into durable access.

**Distinct challenges.** `options.challenge` and `uvOptions.challenge` **MUST** differ and **MUST** both be bound to the `enrollmentId`. Sharing a value collapses two ceremonies into one, and an attacker who can drive either can then satisfy both.
