...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/auth/step-up/approve-response/0.3`, with itself (the **approver**) as `issuer` — the subject in self step-up, or a distinct authorized approver in delegated step-up — and the relying party as `recipient`.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/auth/step-up/approve-response/0.3",
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/auth/step-up/approve-response/0.3#response",
...
The request carries the approver's decision, the echoed `subject`/`sessionId`/
`challenge`, an optional free-text `deniedReason`, and the gate. It carries no
