...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/auth/step-up/approve-response/0.2`, with itself as `issuer` and the relying party as `recipient`.
...
4. Verify the gate:
   - **`didSigned`** — verify the framework `proof`. Verify `payload.subject` equals the session's subject AND equals the document's `issuer` AND equals the DID resolved from the proof's `verificationMethod`. Mismatch → `subjectMismatch`. Missing/invalid proof → `proofInvalid` (or `noGate` if no gate of either kind is present).
