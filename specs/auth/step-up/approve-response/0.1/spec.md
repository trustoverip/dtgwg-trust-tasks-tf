...
4. Verify the gate:
   - **`did-signed`** — verify the framework `proof`. Verify `payload.subject` equals the session's subject AND equals the document's `issuer` AND equals the DID resolved from the proof's `verificationMethod`. Mismatch → `subjectMismatch`. Missing/invalid proof → `proofInvalid` (or `noGate` if no gate of either kind is present).
