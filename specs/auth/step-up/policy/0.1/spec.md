...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/auth/step-up/policy/0.1`, with the administrator as `issuer` and the maintainer as `recipient`.
...
3. Compute **effective mode** = the *strictest* of `floor.mode` and `e.stepUp.require` (absent override ⇒ just `floor.mode`). Strictness order: `none` < `self` < `delegated-any` < `delegated`. An override weaker than the floor is ignored (additive-only).
...
6. Otherwise resolve a **method**:
   - `delegated` → the `approver` on `e.stepUp`. `self` → the caller's own authenticator. `delegated-any` → any VID meeting the maintainer's approver criterion.
...
* **Mode.** The required step-up shape: `none` (AAL1) | `self` | `delegated` | `delegated-any`.
