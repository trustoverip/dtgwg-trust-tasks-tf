...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/auth/step-up/approve-request/0.2`, with itself as `issuer` and the approver as `recipient`.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/auth/step-up/approve-request/0.2#response",
