...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/trust-task-error/0.3`, with itself as `issuer` and the original task's producer as `recipient`.
