...
category: identity
keywords: [persona, privacy, correlation, world]
authors:
...
> **1.1 renames three response members and nothing else.** `facetId` is `worldId`,
> `facetIds` is `worldIds`, and `crossesFacets` is `crossesWorlds` — the same values,
> under the word the rest of the family uses for an arrangement of faces. `facet`
> shared a stem with `face` while naming something different, which is a collision a
> holder should never have had to hold in their head. A breaking rename, permitted as a
> MINOR increment because this specification is `draft` ([SPEC.md §5.2](/SPEC.md#52-compatibility-rules)).
...
**Severity is how linkable. Worlds are whether the holder minds.** These are two
...
holder intended. `crossesWorlds` is a fact about the holder's own arrangement: a
...
them the one that matters. The same value shared *across* worlds is the finding
worth raising, and it is the reason a maintainer that implements
`persona/world/*` can say something here that one without worlds cannot.

A maintainer **MUST NOT** reduce `severity` because a linkage stays inside one
world. The linkage is real either way: two verifiers who see both profiles link
...
A maintainer that implements `persona/world/*` **SHOULD** populate
`crossesWorlds`, `worldIds` and `sharedWith[].worldId`; one that does not
...
implementation with no worlds has made no such finding. A consumer **MUST**
treat an absent `crossesWorlds` as *unknown* and not as *does not cross*.

A maintainer **MUST NOT** reduce `severity` because a linkage falls inside a
single world, and **MUST NOT** omit a finding for that reason. The arrangement
...
`crossesWorlds` is true only where two or more **distinct** worlds appear among
`sharedWith`. A profile belonging to no world is unarranged, not a second world,
...
### The world members carry the holder's own filing

`worldIds` and `sharedWith[].worldId` say which parts of their life the holder
considers these identities to belong to — the most correlating statement in the
persona family, and the reason `persona/world/*` never leaves the agent scope.
...
maintainer **MUST NOT** return a world **name** in this response — the caller
holds the world records and can resolve an identifier, and a name is the member
of a world worth protecting.
