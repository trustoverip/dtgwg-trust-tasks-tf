---
slug: persona/world/put
version: "1.0"
title: "Persona — World Put"
...
proofRequirement:
  requirement: REQUIRED
  rationale: A world names records that sit above every trust context, so a document that could compose one without attribution would let anything holding a session rearrange the holder's own account of their life. Attribution must survive the transport for the same reason it must for the profiles a world groups.
issuedAtRequirement:
  requirement: REQUIRED
  rationale: A put replaces a world's membership wholesale, so two copies applied out of order leave the older arrangement in place — and the older arrangement may place a face somewhere the holder has since moved it from.
sideEffects:
  level: mutating
  rationale: >-
    Creates or replaces one world. Recoverable, and unusually so — no profile
...
errorCodes:
  - code: persona/world/put:faceAlreadyPlaced
    meaning: One or more of the listed `faceIds` already belongs to a different world. The details name each offending profile and the world currently holding it, so a producer can offer to move it rather than guessing. The world is not written.
    retryable: false
    detailsSchema:
      type: object
  ...
  - code: persona/world/put:unresolvedReference
    meaning: A listed `faceId` or `attributeId` names a record the holder does not hold. The details name them. The world is not written — an arrangement referring to something that never existed is a typo, and accepting it silently makes the typo permanent.
    retryable: false
    detailsSchema:
      type: object
      additionalProperties: false
...
**Persona World Put** names a part of a holder's life and says what belongs to
it.
...
nobody reads. A world is the arrangement over them: *Work*, *Home*, *Play*, and
...
The authority is **ownership of the pool being arranged**. A world groups
...
trust context is **not** authorized here: a world names records belonging to
...
**World** — a named part of the holder's life, and the profiles and attributes
that belong to it. It is an *arrangement*, not a container: nothing is stored
inside a world, and deleting one deletes nothing but the arrangement.

**`worldId`** — the record's identity, assigned by the maintainer on create.
...
**`faceIds`** — the profiles belonging to this world. **A profile belongs to at
most one world.** A maintainer **MUST** refuse a document placing a profile that
another world already holds, with `persona/world/put:faceAlreadyPlaced`.

**`attributeIds`** — the attributes belonging to this world. **An attribute MAY
...
### Why membership lives on the world

The obvious alternative is a `worldId` member on `persona/attribute/put` and
...
place forty attributes into a world would have to send forty puts, each of which
...
### Creating a world with two profiles and a shared attribute
...
supplied its own `worldId` cannot otherwise tell. Failures use `trust-task-error`
...
world is not thinking about a maintainer's logs. A maintainer **MUST NOT** write
...
`name` or `icon` — those members exist to find a world again, and any other use
...
may be added later, and a producer building a world before the holder has
decided what goes in it **SHOULD** send exactly that.

### Correlation

A world is the holder's own statement that two identities belong to the same
...
stays there**. A world, its name, its colour and its membership **MUST NOT**
...
Within the agent, a world makes an existing correlation report sharper rather
than adding a new exposure: a maintainer that already indexes shared values
(`persona/correlation/analyze`) can distinguish sharing *inside* a world, which
the holder intended, from sharing *across* worlds, which is the finding worth
...
A world is durable by nature: it is an arrangement the holder made and expects
to find again. A maintainer keeps it until the holder deletes it
(`persona/world/delete`). It has no evidentiary value — nothing about a world is
...
world for anything else — inferring a policy scope from it, grouping audit
records by it, treating membership as authority — is outside the purpose it was
collected for. A world is not a permission and a maintainer **MUST NOT** read it
as one.
