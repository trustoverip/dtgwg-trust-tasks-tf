...
* Sort `supportedTypes` lexicographically for deterministic output.
* Include `targetFrameworkVersion` ≥ the discoverer's framework version where it has a choice — listing specifications whose target framework version is older than the discoverer's permits forward-minor-compatible processing per [SPEC.md §5.2](../../../SPEC.md#52-compatibility-rules).
