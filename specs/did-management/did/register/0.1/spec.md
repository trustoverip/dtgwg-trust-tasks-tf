...
Register is also the completion step for a **reserved slot**: after [`did-management/did/check-name`](../../check-name/0.1/spec.md) with `reserve: true`, the owner registers against the reserved `path` and the log lands as the slot's first published content (`versionCount` advances from `0`). The separate [`did-management/did/publish`](../../publish/0.1/spec.md) task that used to carry this second step is **retired** — register fully covers the reserved-slot flow, because a register from the slot's current owner is always accepted as an update rather than a collision. One-shot `register` is preferred when the caller has the signed log ready at submit time — it avoids the resolvability gap where a reserved-but-not-yet-published path returns 404 to any in-flight resolver; reserve-then-register remains available when the path must be pinned before the log can be produced (e.g. the SCID derivation needs the final URL).

The task is **idempotent for the slot's owner**: re-submitting the same `path` + signed log against an unchanged record is a no-op. A second call from the same owner with a *new* log entry is an update — `versionCount` advances, the new log replaces the old, and the response carries the new state. This owner-update rule is what subsumes the retired publish task: "publish to my reserved (or existing) slot" and "re-register my slot with a new log" are the same operation.
