...
**Projected right of a role** — the right that the namespace's bridge maps to a given forge role on this repository: the inverse of the mapping `desiredRoles` uses in `git-ns/bridge/job` 0.4 (trust-tasks #635), under the role map the bridge applies to the repository — the one it reports with `git-ns/bridge/event` 0.3's `roleMapReported`, or the default map where it has reported none. It is the **lowest** right whose role is the given one, so where two rights project to the same role the projected right is the lower one: on a personal account, where `own` and `maintain` both collapse to collaborator `write`, `write` projects `git.repo.maintain`, never `git.repo.own`. The forge there cannot tell an owner from a maintainer, so separation of duties is enforced at the VTC, not the forge. No role map gives `git.repo.maintain` or `git.commit.sign` the forge's administrator or owner role — only `git.repo.own` may map to it (`git-ns/bridge/job` 0.4 refuses any other map) — so on an organisation the administrator role, where any right projects to it, projects `git.repo.own`. With the default map on a GitHub organisation, `admin` is the projection of `git.repo.own` and `maintain` of `git.repo.maintain`; `write` is the projection of `git.commit.sign` only on a repository that projects its committers to `write`, and of nothing otherwise. A role that no right projects to (`triage`, `read`) has none. `git.ns.admin` and `git.repo.create` are never a projected right: `git.ns.admin` itself never projects a forge role, and neither does `git.repo.create`. A namespace admin may still hold ordinary, non-elevated rights in their own name — `git.repo.maintain`, `git.commit.sign` — which project to non-admin forge roles as anyone's do; granting those to oneself is an allowed self-grant.

**Projected right of a member** — the one right the member is projected at on this repository, as `desiredRoles` lists them from `git-ns/bridge/job` 0.4 on: the highest of the rights recorded **in their own name** that reach it — `git.repo.own`, `git.repo.maintain` or `git.commit.sign` on the repository, or `git.commit.sign` on its namespace — in the order `git.commit.sign` < `git.repo.maintain` < `git.repo.own`. What `git.ns.admin` implies does not count: a namespace admin projects to no forge role, so a namespace admin holding nothing in their own name on the repository has no projected right there. This is defined here in full, rather than by reference, because `git-ns/bridge/job` 0.3 and earlier list a member at their highest *effective* right, which counts a namespace admin as an owner; a VTC applies this definition whichever job version its bridge implements. It is what the forge is meant to show for the member, which is what a `roleChanged` item differs from — and so, unlike the member's effective rights, what an adoption can raise.
...
7. Evaluates the right exactly as `git-ns/right/grant` 0.3 evaluates a grant sent by the resolver with `subject` this request's `subject`, `right` the projected right, `resource` the repository, no `expiresAt`, and `reason` this request's `reason`: its request steps 3 to 7, including the fixed rules of `git-ns/right/grant` 0.3 in order, policy, the idempotent return of a live record that already exists, and `grantedBy` set to the resolver. A refusal there is this task's refusal, with the same code. The VTC **MUST** make the checks of steps 2 and 3, and the selection of the item, again under the same exclusion as the write that records the right, and refuse as those steps do if either no longer holds: a relink or a new drift report between the first check and the write adopts nothing.
