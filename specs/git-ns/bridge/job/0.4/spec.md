...
These are breaking changes, released as a `MINOR` increment under the `draft` allowance of [SPEC §5.2](/SPEC.md#52-compatibility-rules). A `0.3` document is a valid `0.4` document exactly when every `projectRoles` job names `repo`, no account appears twice in `desiredRoles`, and no `desiredRoles` entry lists a right the person holds only by implication. A VTC **MUST NOT** send a `0.4` job to a bridge that has not shown it takes `0.4` — a `0.3` bridge would read a `git.ns.admin` entry as ownership — and **SHOULD** treat the bridge's `unsupportedType` or `unsupportedVersion` refusal of a `0.4` job as that bridge taking only earlier versions. Everything else is unchanged from `0.3` and restated below, so that this version stands on its own.
