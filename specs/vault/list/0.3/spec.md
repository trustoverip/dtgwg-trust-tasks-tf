...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/vault/list/0.3`, with itself as `issuer` and the vault maintainer as `recipient`.
...
* **Metadata view.** A `VaultEntry` with `secretKind` populated but no secret material; defined in [`_shared/0.3/vault-entry.schema.json`](../../_shared/0.3/vault-entry.schema.json).
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vault/list/0.3",
...
A success *response* document carries `type: https://trusttasks.org/spec/vault/list/0.3#response`. Payload: `{ entries, truncated, cursor?, redactedFields? }`.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vault/list/0.3#response",
