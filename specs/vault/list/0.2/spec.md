...
1. Emit a *Trust Task document* whose `type` is `https://trusttasks.org/spec/vault/list/0.2`, with itself as `issuer` and the vault maintainer as `recipient`.
...
* **Metadata view.** A `VaultEntry` with `secretKind` populated but no secret material; defined in [`_shared/0.2/vault-entry.schema.json`](../../_shared/0.2/vault-entry.schema.json).
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vault/list/0.2",
...
A success *response* document carries `type: https://trusttasks.org/spec/vault/list/0.2#response`. Payload: `{ entries, truncated, cursor?, redactedFields? }`.
...
```json
{
  ...
  "type": "https://trusttasks.org/spec/vault/list/0.2#response",
    ...
    "entries": [
      {
        ...
        "contextId": "ctx_work",
        "targets": [
          { "kind": "webOrigin", "origin": "https://github.com" },
          { "kind": "iosApp", "bundleId": "com.github.stwalkerster.codehub", "teamId": "VEKTX9H2N7" },
          { "kind": "androidApp", "packageName": "com.github.android",
      ...
      },
      {
        ...
        "contextId": "ctx_work",
        "targets": [
          { "kind": "webOrigin", "origin": "https://aws.amazon.com" }
        ],
        "label": "Work AWS — root",
        "secretKind": "password",
