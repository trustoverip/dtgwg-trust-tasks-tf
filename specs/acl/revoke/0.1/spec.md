...
The `acl/revoke:lastAuthorityProtected` refusal is **not** an authorization check and must not be mistaken for one. It fires after the caller has been authorized, and protects the ACL from being left with no holder of a privileged role — a policy invariant about the resulting state, not about who asked.
