...
* *Party* — An entity that participates in a *Trust Task*. Each party is identified by a Decentralized Identifier (DID) [[DID-CORE]].
...
* *Document identifier* — The URI carried in the `id` member of a *Trust Task document* that uniquely identifies that instance.
...
|---|---|---|---|
| `id` | **MUST** | string (URI) | The *Document identifier* — a globally unique URI for this instance of the task. See [§4.3](#43-the-id-member). |
...
| `issuer` | **MAY** | string (URI) | A URI (typically a DID) identifying the *party* responsible for the document's content. See [§4.8](#48-the-issuer-and-recipient-members). |
| `recipient` | **MAY** | string (URI) | A URI (typically a DID) identifying the *party* the *issuer* expects to act upon the document. See [§4.8](#48-the-issuer-and-recipient-members). |
...
> ```json
> {
>   "id": "did:web:verifier.example#kyc-handoff/4f3c",
...
The `id` member's value **MUST** be a URI [[RFC3986]] that is globally unique to this instance of the task. Producers **SHOULD** use a DID URL or other resolvable URI scheme that allows the document to be located after issuance. The value of `id` **MUST NOT** be reused across documents.
...
The framework separates document-level metadata (`id`, `type`, `issuer`, `recipient`, `issuedAt`, `expiresAt`, `proof`) from task-specific data (`payload`) so that a single framework-level schema validates the outer structure, with per-task schemas applied only to `payload`. Schema scope is defined in [§6.3](#63-schema-scope).
...
A *Trust Task document* **MAY** include a `proof` member whose value is a JSON object describing an integrity proof over the document. The internal structure of `proof` is out of scope for this framework revision; concrete proof formats — for example, JOSE-based signatures or [[VC-DATA-INTEGRITY]] proofs — **MAY** be specified by individual *Trust Task specifications* or by a future revision of this framework.

When `proof` is present, it covers the document with `proof` itself excluded from the signed content, in the manner conventional for detached-style proofs over JSON.
...
* `issuer` — a URI (typically a DID) identifying the *party* responsible for the document's content. When `proof` is present, the `issuer` **MUST** be the entity to which the proof verifies.
* `recipient` — a URI (typically a DID) identifying the *party* the *issuer* expects to act upon the document.
...
The outer document structure (`id`, `type`, `issuer`, `recipient`, `issuedAt`, `expiresAt`, `payload`, `@context`, `proof`) is described by the JSON Schema served at the framework's own *Type URI* — `https://trusttasks.org/spec/trust-task/<MAJOR.MINOR>` — under content negotiation for `application/schema+json`. A complete document validation therefore composes the framework schema (outer structure) with the task-specific payload schema.
...
An *error response* is a *Trust Task document* whose `type` is the URI above. Its `payload` carries the standard error structure defined in [§8.2](#82-error-payload). The `id` member of an *error response* identifies the error instance and **MUST NOT** be reused; the original task being responded to is referenced from within the payload by `inResponseTo`.

### 8.2 Error payload

The `payload` of an *error response* has the following members.

| Member | Required | Type | Description |
|---|---|---|---|
| `inResponseTo` | **MUST** | string (URI) | The `id` of the *Trust Task document* this error reports on. |
...
> ```json
> {
>   "id": "did:web:bank.example#error/9e2a",
...
>   "payload": {
>     "inResponseTo": "did:web:verifier.example#kyc-handoff/4f3c",
>     "code": "expired",
